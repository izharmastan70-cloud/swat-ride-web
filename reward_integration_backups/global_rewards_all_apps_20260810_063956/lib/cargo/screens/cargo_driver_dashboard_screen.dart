  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<CargoDriverApplicationModel>>(
      ...
      builder: (
        ...
        for (final CargoDriverApplicationModel application in applications) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: StreamBuilder<List<CargoBookingModel>>(
          stream: bookingService.watchDriverBookings(
            application.userId,
          ),
          builder: (
            BuildContext context,
            ...
            final List<CargoBookingModel> activeBookings =
                bookings.where((CargoBookingModel booking) {
              return booking.status != CargoBookingModel.delivered &&
                  booking.status != CargoBookingModel.cancelled;
            }).toList();

            final List<CargoBookingModel> completedBookings =
                bookings.where((CargoBookingModel booking) {
              return booking.status == CargoBookingModel.delivered;
            }).toList();
...
class _CargoJobCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String drop =
        booking.dropAddress?.trim().isNotEmpty == true
...
class _AccessStatusScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (applications.isNotEmpty) {
      final CargoDriverApplicationModel latest =
          applications.first;

      if (latest.status ==
          CargoDriverApplicationModel.pending) {
        title = 'Application under review';
        message =
            'Your Cargo Driver application is waiting for admin approval.';
      } else if (latest.status ==
          CargoDriverApplicationModel.rejected) {
        title = 'Application rejected';
        message =
            latest.adminNote?.trim().isNotEmpty == true
                ? latest.adminNote!.trim()
                : 'Your Cargo Driver application was not approved.';
        icon = Icons.cancel_outlined;
      } else if (latest.status ==
          CargoDriverApplicationModel.suspended) {
