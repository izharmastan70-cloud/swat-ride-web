...
} from '../../phase66_post_rebind_enable_offline/src/trusted_post_rebind_enable_transaction_body.mjs';

const POST_REBIND_ENABLE_ARMING_TOKENS =
  'agent_production_rollout_arming_tokens';
const POST_REBIND_ENABLE_REBIND_RECEIPTS =
  'agent_security_incident_post_migration_rebind_receipts';

export const POST_REBIND_ENABLE_LIVE_DEFAULTS = Object.freeze({
  executionArmed: false,
  executionWord: '',
  allowAdminSdkInit: false,
  allowCredentialLoad: false,
  ...
  allowApprovalConsume: false,
  allowTokenMutation: false,
  allowRoleEnable: false,
  allowMigrationHoldRelease: false,
  allowAuditWrite: false,
  allowRepositoryAttach: false,
  allowRepositoryArm: false,
  allowIncidentWrite: false,
...
function assertPostRebindEnableTokenIssueArmed(config) {
  if (config?.executionArmed !== true) {
    fail('post_rebind_enable_token_issue_not_armed');
  }

  assertExecutionWord(
    config,
    'PHASE66_POST_REBIND_ENABLE_REPLACEMENT_TOKEN_ISSUE_ONLY',
  );
  ...
  for (const key of requiredTrue) {
    if (config?.[key] !== true) {
      fail(`post_rebind_enable_token_issue_required_flag_missing:${key}`);
    }
  }
  ...
  for (const key of requiredFalse) {
    if (config?.[key] !== false) {
      fail(`post_rebind_enable_token_issue_forbidden_flag_enabled:${key}`);
    }
  }
}

function assertPostRebindEnableAtomicArmed(config) {
  if (config?.executionArmed !== true) {
    fail('post_rebind_enable_atomic_not_armed');
  }

  assertExecutionWord(
    config,
    'PHASE66_POST_REBIND_ENABLE_AND_HOLD_RELEASE_ONLY',
  );
  ...
  for (const key of requiredTrue) {
    if (config?.[key] !== true) {
      fail(`post_rebind_enable_atomic_required_flag_missing:${key}`);
    }
  }
  ...
  for (const key of requiredFalse) {
    if (config?.[key] !== false) {
      fail(`post_rebind_enable_atomic_forbidden_flag_enabled:${key}`);
    }
  }
}

function assertReplacementTokenCredential(
  rawReplacementToken,
  replacementTokenIdSha256,
) {
  if (
    typeof rawReplacementToken !== 'string' ||
    rawReplacementToken.length < 32 ||
    !isSha256(replacementTokenIdSha256)
  ) {
    fail('replacement_token_credential_invalid');
  }

  const actual =
    sha256Hex(rawReplacementToken);

  if (
    lower(actual) !==
    lower(replacementTokenIdSha256)
  ) {
    fail('replacement_token_raw_credential_sha_mismatch');
  }

  return true;
}

function validatePostRebindEnableCommonInput({
  approvalId,
  requesterReferenceSha256,
  ownerApproverReferenceSha256,
  ...
  if (!norm(approvalId)) {
    fail('post_rebind_enable_approval_id_required');
  }
  ...
  for (const [field, value] of Object.entries({
    ...
    if (!isSha256(value)) {
      fail(`post_rebind_enable_${field}_invalid`);
    }
  }
  ...
  if (
    ...
    fail('post_rebind_enable_requester_owner_must_differ');
  }
}

function postRebindEnableRefs({
  db,
  approvalId,
  rebindReceiptIdSha256,
  replacementTokenIdSha256,
  historicalTokenIdSha256,
  includeHistoricalToken,
  includeTokenAudit,
  includeEnableAudit,
}) {
  const approvals =
  ...
  const armingTokens =
    db.collection(POST_REBIND_ENABLE_ARMING_TOKENS);

  const rebindReceipts =
    db.collection(POST_REBIND_ENABLE_REBIND_RECEIPTS);

  const auditLogs =
    db.collection(AGENT_AUDIT_LOGS);

  const refs = {
    approval: approvals.doc(approvalId),
    manifest:
      settings.doc(
    ...
    guard:
      settings.doc('production_rollout_guard'),
    rollout:
      settings.doc('production_rollout'),
    role:
      roles.doc(SECURITY_INCIDENT_ROLE_ID),
    rebindReceipt:
      rebindReceipts.doc(
        lower(rebindReceiptIdSha256),
      ),
    replacementToken:
      armingTokens.doc(
        lower(replacementTokenIdSha256),
  ...
  if (includeHistoricalToken) {
    if (!isSha256(historicalTokenIdSha256)) {
      fail('post_rebind_enable_historical_token_id_invalid');
    }

    refs.historicalToken =
      armingTokens.doc(
        lower(historicalTokenIdSha256),
      );
  }

  if (includeTokenAudit) {
    refs.tokenAudit = auditLogs.doc();
  }

  if (includeEnableAudit) {
    refs.enableAudit = auditLogs.doc();
  }
...
export async function executePostRebindEnableReplacementTokenIssue({
  config = POST_REBIND_ENABLE_LIVE_DEFAULTS,
  approvalId,
  requesterReferenceSha256,
  ownerApproverReferenceSha256,
  rebindReceiptIdSha256,
  ...
  validatePostRebindEnableCommonInput({
    approvalId,
    requesterReferenceSha256,
    ownerApproverReferenceSha256,
    rebindReceiptIdSha256,
  ...
  if (
    ...
    fail('post_rebind_enable_token_ttl_out_of_bounds');
  }
  ...
  try {
    ...
    const refs =
      postRebindEnableRefs({
        db,
        approvalId,
        rebindReceiptIdSha256,
        replacementTokenIdSha256,
        historicalTokenIdSha256,
        includeHistoricalToken: true,
        includeTokenAudit: true,
        includeEnableAudit: false,
      });
  ...
  try {
    ...
    const refs =
      postRebindEnableRefs({
        ...
        rebindReceiptIdSha256,
        replacementTokenIdSha256,
        historicalTokenIdSha256: null,
        includeHistoricalToken: false,
        includeTokenAudit: false,
        includeEnableAudit: true,
      });
...
export const POST_REBIND_ENABLE_LIVE_EXECUTOR_CONTRACT =
  Object.freeze({
    ...
    defaultExecutionArmed: false,
    replacementTokenIssueExecutionWord:
      'PHASE66_POST_REBIND_ENABLE_REPLACEMENT_TOKEN_ISSUE_ONLY',
    atomicEnableExecutionWord:
      'PHASE66_POST_REBIND_ENABLE_AND_HOLD_RELEASE_ONLY',
    replacementTokenRawCredentialRequired: true,
    replacementTokenRawCredentialPersisted: false,
    replacementTokenRawCredentialPrinted: false,
    replacementTokenIssueExactWrites: 2,
    atomicEnableExactWrites: 5,
    approvalRisk: 'HIGH',
    approvalConsumeInsideAtomicEnable: true,
    replacementTokenConsumeInsideAtomicEnable: true,
    roleEnableInsideAtomicEnable: true,
    migrationHoldReleaseInsideAtomicEnable: true,
    roleModeAfterEnable: 'ASK_FIRST',
    rolloutStageAfterEnable: 'MONITOR_ONLY',
    repositoryAttachAuthorized: false,
    repositoryArmAuthorized: false,
    firstIncidentWriteAuthorized: false,
    suggestOnlyAuthorized: false,
