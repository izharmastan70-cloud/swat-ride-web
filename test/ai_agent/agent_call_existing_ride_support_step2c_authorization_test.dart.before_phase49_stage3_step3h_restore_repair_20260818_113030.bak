...
import 'package:swat_ride/ai_agent/services/agent_call_existing_ride_read_authorization_service.dart';
...
AgentRole _rideRole() {
  return buildInitialAgentRoles().firstWhere(
    (AgentRole role) => role.roleId == 'ride_agent',
  );
}
...
void main() {
  group('Phase 49 Stage 2 Step 2C existing Ride read authorization', () {
    const service = AgentCallExistingRideReadAuthorizationService();

    test('dedicated Call existing-Ride read action is registered', () {
      expect(
        AgentActionId.values,
        contains(AgentActionId.readCallExistingRide),
      );
    ...
    test(
      'call_agent remains AUTO and least privilege with exactly two actions',
      () {
    ...
    test(
      'valid trusted read passes central Permission Engine and Runtime Gate',
      () {
    ...
    test('AI master OFF blocks read', () {
      final result = service.authorize(
    ...
    test('Call Agent master switch OFF blocks read', () {
      final result = service.authorize(
    ...
    test('Free AI switch OFF blocks free-ai Call role', () {
      final result = service.authorize(
    ...
    test(
      'Emergency read-only blocks AUTO Call role under existing Runtime Gate',
      () {
        final result = service.authorize(
    ...
    test('wrong role/module fails closed before permission use', () {
      final result = service.authorize(
        settings: _settings(),
        role: _rideRole(),
        request: _request(),
    ...
    test('missing trusted caller binding fails closed', () {
      final result = service.authorize(
        settings: _settings(),
        role: _callRole(),
    ...
    test('authorization is read-only and messages never grant authority', () {
      ...
      expect(service.queryCanGrantPermission, isFalse);
      ...
      expect(service.rideReferenceAloneCanGrantPermission, isFalse);
      ...
      expect(service.invokesRideService, isFalse);
