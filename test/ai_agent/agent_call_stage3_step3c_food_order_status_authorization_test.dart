...
import 'package:swat_ride/ai_agent/services/agent_call_food_order_status_authorization_service.dart';
...
AgentRole _foodRole() {
  return buildInitialAgentRoles().firstWhere(
    (AgentRole role) => role.roleId == 'food_agent',
  );
}
...
void main() {
  group('Phase 49 Stage 3 Step 3C Food status authorization', () {
    const AgentCallFoodOrderStatusAuthorizationService service =
        AgentCallFoodOrderStatusAuthorizationService();

    test(
      'dedicated Call Food status action is registered low-risk read-only',
      () {
        expect(
          AgentActionId.values,
          contains(AgentActionId.readCallFoodOrderStatus),
        );
        ...
        expect(definition.actionId, AgentActionId.readCallFoodOrderStatus);
    ...
    test('call_agent has exactly three dedicated Call actions', () {
      ...
      expect(
        role.allowedActions,
        isNot(contains(AgentActionId.refundFoodOrder)),
      );
      expect(
        role.allowedActions,
    ...
    test('complete trusted Food order binding validates safely', () {
      final AgentCallFoodOrderStatusRequest request = _request();
      ...
      expect(map['trustedOrderReferenceId'], 'order-ref-1');
      ...
      expect(map['orderReadExecuted'], isFalse);
      expect(map['foodWriteAuthority'], isFalse);
    });

    test('missing trusted order binding fails closed', () {
      ...
      expect(result.reason, 'TRUSTED_FOOD_ORDER_BINDING_REQUIRED');
    });
    ...
    test('AI master OFF blocks dedicated Food status authorization', () {
      final result = service.authorize(
    ...
    test(
      'Call Agent switch OFF blocks dedicated Food status authorization',
      () {
        final result = service.authorize(
    ...
    test('Food Agent role cannot impersonate Call Agent authority', () {
      final result = service.authorize(
        settings: _settings(),
        role: _foodRole(),
        request: _request(),
    ...
    test('authorization itself executes no Food read/write capability', () {
      ...
      expect(service.genericFoodActionCanGrantCallPermission, isFalse);
      ...
      expect(service.orderReferenceAloneCanGrantPermission, isFalse);

      expect(service.executesFoodConnector, isFalse);
      expect(service.readsFoodOrder, isFalse);
      expect(service.writesFoodOrder, isFalse);
      expect(service.createsFoodOrder, isFalse);
      expect(service.cancelsFoodOrder, isFalse);
      expect(service.refundsFoodOrder, isFalse);
      expect(service.changesPayment, isFalse);
      expect(service.assignsRider, isFalse);

      expect(service.invokesFirestore, isFalse);
      expect(service.invokesFirebaseAuth, isFalse);
      expect(service.invokesFoodOrderService, isFalse);
