...
class RestaurantPartnerDashboardScreen extends StatefulWidget {
  const RestaurantPartnerDashboardScreen({required this.partner, super.key});
...
class _RestaurantPartnerDashboardScreenState
  ...
  final RestaurantPartnerService _partnerService = RestaurantPartnerService();

  final FoodOrderService _orderService = FoodOrderService();

  final RestaurantPartnerAnalyticsService _analyticsService =
      RestaurantPartnerAnalyticsService();

  final RestaurantPartnerNotificationService _notificationService =
      RestaurantPartnerNotificationService();

  final RestaurantPartnerSettlementService _settlementService =
  ...
  Future<void> _toggleRestaurantStatus(bool isOpen) async {
    ...
    if (!_hasRestaurant) {
      _showMessage('Approved restaurant record is not available.');
      return;
    }
    ...
    try {
      ...
      _showMessage('Unable to update restaurant status: $error');
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _openNamedRoute(String routeName, {Object? arguments}) {
    Navigator.pushNamed(context, routeName, arguments: arguments ?? partner);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          const RewardAccessAction(moduleName: 'restaurantPartner'),
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: _notificationService.watchUnreadNotifications(
              partner.partnerId,
            ),
            builder:
                (
          ...
          IconButton(
            tooltip: 'Profile',
            onPressed: () {
              _openNamedRoute('/food_partner_profile');
            },
      ...
      body: SafeArea(
        child: StreamBuilder<List<FoodOrderModel>>(
          stream: _hasRestaurant
              ? _orderService.watchRestaurantOrders(partner.restaurantId)
              : Stream<List<FoodOrderModel>>.value(const <FoodOrderModel>[]),
          builder:
              (
                BuildContext context,
                ...
                final _DashboardStats stats = _DashboardStats.fromOrders(
                  orders,
                );
      ...
      child: Row(
        children: <Widget>[
          const CircleAvatar(
            ...
            child: Icon(Icons.restaurant_menu, color: yellow, size: 34),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  '${partner.city} Ã¢â‚¬Â¢ ${partner.applicationStatus.displayName}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
  ...
  Widget _buildOpenCloseCard() {
    return StreamBuilder<RestaurantPartnerModel?>(
      stream: _partnerService.watchPartnerById(partner.partnerId),
      initialData: partner,
      builder:
          (
            BuildContext context,
            ...
            return Container(
              ...
              child: Row(
                children: <Widget>[
                  CircleAvatar(
                    backgroundColor: currentPartner.isActive
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Icon(item.icon, color: yellow, size: 27),
              Text(
                item.value,
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
  ...
  Widget _buildBusinessOverview() {
    if (!_hasRestaurant) {
      return _buildInfoMessage(
        ...
        message: 'The approved restaurant record is not connected yet.',
      );
    }

    return FutureBuilder<List<Object>>(
      future: Future.wait<Object>(<Future<Object>>[
        _analyticsService.dashboardSummary(partner.restaurantId),
        _settlementService.pendingAmount(partner.partnerId),
        _settlementService.paidAmount(partner.partnerId),
      ]),
      builder: (BuildContext context, AsyncSnapshot<List<Object>> snapshot) {
        ...
        if (snapshot.hasError ||
          ...
          return _buildInfoMessage(
            ...
            message: 'Check Firestore rules, indexes and internet connection.',
          );
        }

        final Map<String, dynamic> analytics = Map<String, dynamic>.from(
          snapshot.data![0] as Map,
        );

        final double pendingSettlement = snapshot.data![1] as double;

        final double paidSettlement = snapshot.data![2] as double;

        final double averageOrderValue = _numberValue(
          analytics['averageOrderValue'],
        );

        final int cancelledOrders = _intValue(analytics['cancelledOrders']);
        ...
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const Row(
                children: <Widget>[
                  Icon(Icons.analytics_outlined, color: yellow),
                  SizedBox(width: 9),
                  Text(
                    'Business Overview',
              ...
              _OverviewRow(
                ...
                value: 'Rs. ${pendingSettlement.toStringAsFixed(0)}',
                highlight: true,
              ),
              const Divider(color: Colors.white12),
              _OverviewRow(
                label: 'Paid settlement',
                value: 'Rs. ${paidSettlement.toStringAsFixed(0)}',
              ),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: const Text('Open Earnings & Settlements'),
  ...
  Widget _buildManagementGrid() {
    final List<_DashboardAction> actions = <_DashboardAction>[
      ...
      _DashboardAction(
        ...
        routeName: '/food_partner_disputes',
      ),
      _DashboardAction(
        title: 'Menu',
        subtitle: 'Items and categories',
        icon: Icons.restaurant_menu,
        routeName: '/food_partner_menu_management',
      ),
      _DashboardAction(
        title: 'Add Food Item',
        subtitle: 'Create new dish',
        icon: Icons.add_circle_outline,
        routeName: '/food_partner_add_menu_item',
      ),
      _DashboardAction(
        title: 'Earnings',
        subtitle: 'Sales and commission',
        icon: Icons.account_balance_wallet_outlined,
        routeName: '/food_partner_earnings',
      ),
      _DashboardAction(
        title: 'Reviews',
        subtitle: 'Ratings and feedback',
        icon: Icons.star_outline,
        routeName: '/food_partner_reviews',
      ),
      _DashboardAction(
        title: 'Profile',
        subtitle: 'Restaurant details',
        icon: Icons.store_mall_directory_outlined,
        routeName: '/food_partner_profile',
      ),
      _DashboardAction(
        title: 'Settings',
        subtitle: 'Business controls',
        icon: Icons.settings_outlined,
        routeName: '/food_partner_settings',
      ),
      _DashboardAction(
        title: 'Notifications',
        subtitle: 'Order and admin alerts',
        icon: Icons.notifications_outlined,
        routeName: '/food_partner_notifications',
      ),
    ];
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return Material(
          ...
          child: InkWell(
            onTap: () {
              _openNamedRoute(action.routeName);
            },
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: Column(
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            TextButton(
              onPressed: () {
                _openNamedRoute('/food_partner_orders');
              },
              child: const Text(
                'View All',
        ...
        if (connectionState == ConnectionState.waiting && orders.isEmpty)
          const Center(
            child: Padding(
          ...
          _buildInfoMessage(
            ...
            message: 'Check Firestore connection and rules.',
          )
          ...
          ...recentOrders.map(
            (FoodOrderModel order) => _RecentOrderTile(
              order: order,
              onTap: () {
                Navigator.pushNamed(
                  context,
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          backgroundColor: _statusColor.withValues(alpha: 0.13),
          child: Icon(Icons.receipt_long_outlined, color: _statusColor),
        ),
        title: Text(
        ...
        subtitle: Text(
          '${order.items.length} item(s) Ã¢â‚¬Â¢ Rs. ${order.grandTotal.toStringAsFixed(0)}',
          style: const TextStyle(color: Colors.grey, fontSize: 12),
        ),
        trailing: Container(
...
class _DashboardStats {
  ...
  factory _DashboardStats.fromOrders(List<FoodOrderModel> orders) {
    int newOrders = 0;
    ...
    for (final FoodOrderModel order in orders) {
      switch (order.status) {
        ...
        case FoodOrderStatus.delivered:
          ...
          final double restaurantSale = (order.itemsTotal - order.discount)
