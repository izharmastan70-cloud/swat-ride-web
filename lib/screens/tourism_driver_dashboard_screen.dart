...
class _TourismDriverDashboardScreenState
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        actions: const <Widget>[
          RewardAccessAction(moduleName: 'tourismDriver'),
        ],
        backgroundColor: darkBackground,
        elevation: 0,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${application.vehicleType} ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ${application.vehicleNumber}',
  ...
  void _openUniversalTourismDriverSafety(TourBooking booking) {
    final String driverUserId = _currentUser?.uid.trim() ?? '';

    if (driverUserId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please log in again to open Safety & SOS.'),
        ),
      );
      return;
    }

    final SafetyPersonSnapshot? customer = booking.userId.trim().isEmpty
        ? null
        : SafetyPersonSnapshot(
            userId: booking.userId.trim(),
            role: SafetyUserRole.customer,
          );
    ...
    final List<SafetyPersonSnapshot> relatedPeople = [?customer, ?guide];

    final bool activeTour =
        booking.bookingStatus == 'started' ||
        booking.bookingStatus == 'in_progress' ||
        booking.bookingStatus == 'arrived';

    final SafetyContext safetyContext = SafetyContext(
      serviceType: activeTour
          ? SafetyServiceType.activeTour
          : SafetyServiceType.tourismDriver,
      referenceId: booking.id,
      initiatedByUserId: driverUserId,
      initiatedByRole: SafetyUserRole.tourismDriver,
  ...
  Widget _tourBookingActions({required TourBooking booking}) {
    final Widget statusActions = _tourBookingStatusActions(booking: booking);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        OutlinedButton.icon(
          onPressed: () => _openUniversalTourismDriverSafety(booking),
  ...
  Widget _tourBookingStatusActions({required TourBooking booking}) {
    if (booking.bookingStatus == 'confirmed' ||
        booking.bookingStatus == 'assigned') {
      return Row(
        children: [
