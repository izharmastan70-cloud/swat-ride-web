...
class _HotelBookingDetailsScreenState
    extends State<HotelBookingDetailsScreen> {
  ...
  DateTime get _checkOutDateTime {
    final int hour =
        _readInt(widget.booking['hotelCheckOutHour'], fallback: 12);

    final int minute =
        _readInt(widget.booking['hotelCheckOutMinute']);
          ...
          child: OutlinedButton.icon(
            onPressed: _isWorking ? null : () async {
              await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => HotelCustomerChatScreen(
                    booking: widget.booking,
                  ),
                ),
              );
