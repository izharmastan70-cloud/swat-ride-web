...
class _DriverRideRequestsScreenState
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        '${_paymentName(ride.paymentMethod)} â€¢ '
  ...
  void _openUniversalSafetyCenter(RideModel ride) {
    ...
    final SafetyLocation? currentLocation =
        ride.driverLocation == null
            ...
            : SafetyLocation(
                ...
                placeName: ride.driverLocation!.placeName,
                accuracy: ride.driverLocationAccuracy,
                speed: ride.driverSpeed,
                heading: ride.driverHeading,
                recordedAt: ride.driverLocationUpdatedAt,
              );
    ...
    final SafetyVehicleSnapshot vehicle =
      ...
      registrationNumber: widget.vehicleNumber,
    );
        ...
        builder: (BuildContext context) {
          return SafetyCenterScreen(
            contextData: SafetyContext(
              serviceType: SafetyServiceType.driver,
              referenceId: ride.rideId,
              initiatedByUserId: widget.driverId,
              initiatedByRole: SafetyUserRole.normalDriver,
              sourcePage: ride.status == RideModel.rideStarted
                  ? SafetySourcePage.activeRide
                  : SafetySourcePage.driverDashboard,
              referenceStatus: ride.status,
              vehicle: vehicle,
              currentLocation: currentLocation,
              pickupLocation: pickup,
              destinationLocation: destination,
              serviceTitle: 'SWAT RIDE Driver',
              serviceSubtitle: 'Active ride safety',
  ...
  Future<void> _callPhoneBookingPassenger(RideModel ride) async {
    final String phone = ride.riderPhone?.trim() ?? '';
    if (phone.isEmpty) {
      _showMessage('Passenger phone number is unavailable.', Colors.orange);
      return;
    }
    ...
    if (!opened && mounted) {
      _showMessage('Unable to open the phone dialer.', Colors.red);
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          _buildFareBox(ride),
          if (ride.bookingSource == RideModel.phoneCallBookingSource) ...[
            const SizedBox(height: 14),
            OutlinedButton.icon(
              onPressed: () => _callPhoneBookingPassenger(ride),
              icon: const Icon(Icons.phone_outlined),
              label: Text('CALL ${ride.riderName ?? 'PASSENGER'}'),
              style: OutlinedButton.styleFrom(
