...
// =========================================================
// AI AGENT Ã¢â‚¬â€ AUDIT LOG SERVICE
// =========================================================
//
...
class AgentAuditService {
  ...
  AgentAuditService({FirebaseFirestore? firestore})
  ...
  AgentAuditLog appendInTransaction({
    ...
    final AgentAuditLog log = _buildAtomicLog(
      auditId: doc.id,
      eventType: eventType,
      severity: severity,
      actorType: actorType,
    ...
    transaction.set(doc, log.toMap());
    return log;
  }

  AgentAuditLog appendToBatch({
    ...
    batch.set(doc, log.toMap());
    return log;
  }

  AgentAuditLog _buildAtomicLog({
  ...
  Future<List<AgentAuditLog>> getRecent({int limit = 100}) async {
  ...
  Stream<List<AgentAuditLog>> watchRecent({int limit = 100}) {
    final int safeLimit = limit.clamp(1, 500).toInt();
