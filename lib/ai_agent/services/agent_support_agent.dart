...
import '../../help/services/support_agent_video_guide_lookup_service.dart';
...
import 'agent_support_conversation_quality_adapter.dart';

// =========================================================
// AI AGENT ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SUPPORT AGENT
// =========================================================
//
...
class AgentSupportAgent {
  ...
  final SupportAgentVideoGuideLookupService videoGuideLookup;
  ...
  AgentSupportAgent({
    ...
    SupportAgentVideoGuideLookupService? videoGuideLookup,
       ...
       videoGuideLookup =
           videoGuideLookup ?? SupportAgentVideoGuideLookupService();
  ...
  Future<AgentSupportDraft> draft({
    ...
    // Phase 37 / 37K:
    // Approved Video Library lookup is intentionally AFTER escalation and
    // verified-business-data gates. Tutorial lookup must never bypass them.
    //
    // Failure is non-fatal: customer support continues with the existing
    // deterministic FAQ / Free-Local AI flow.
    HelpVideoTutorialModel? approvedGuide;

    try {
      approvedGuide = await videoGuideLookup.findBestApprovedGuide(
        userQuery: request.userMessage,
        module: request.module,
        feature: request.context['feature']?.toString().trim() ?? '',
        language: request.context['language']?.toString().trim() ?? '',
        currentAppVersion:
            request.context['appVersion']?.toString().trim() ?? '',
        intents: <String>[assessment.intent],
      );
    } catch (_) {
      approvedGuide = null;
    }

    final String approvedGuideHint = approvedGuide == null
        ? ''
        : '\n\nApproved SWAT RIDE video guide: ${approvedGuide.title}';

    final String fallback = faqService.answer(request.userMessage);

    if (!settings.freeAiOperational && !settings.localAiEnabled) {
      return AgentSupportDraft(
        requestId: request.requestId,
        status: AgentSupportDraftStatus.ready,
        replyText: '$fallback$approvedGuideHint',
        intent: assessment.intent,
        priority: assessment.priority,
        escalation: assessment.escalation,
    ...
    final AgentAiRequest aiRequest = AgentAiRequest(
      ...
      context: sanitizer.sanitizeMap(<String, dynamic>{
        'module': request.module,
    ...
    const AgentSupportConversationQualityAdapter conversationQuality =
        AgentSupportConversationQualityAdapter();

    final AgentSupportConversationQualityAdapterResult guardedDraft =
        conversationQuality.guardAiDraft(
          proposedAiReply: response.isSuccess ? response.text.trim() : '',
          deterministicFallback: fallback,
          approvedGuideHint: approvedGuideHint,
        );

    final String reply = guardedDraft.replyText;
    ...
    return AgentSupportDraft(
      ...
      escalation: assessment.escalation,
      canAutoSendLater:
          guardedDraft.allowAutoSendLater && assessment.safeForAutomaticReply,
      note: guardedDraft.note,
    );
  }
}
