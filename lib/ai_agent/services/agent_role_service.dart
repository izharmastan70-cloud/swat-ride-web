...
class AgentRoleService {
  ...
  AgentRole _readSafely(DocumentSnapshot<Map<String, dynamic>> snapshot) {
  ...
  Future<List<AgentRole>> getAllRoles() async {
    ...
    final List<AgentRole> roles = snapshot.docs
        .map(
          (QueryDocumentSnapshot<Map<String, dynamic>> doc) => _readSafely(doc),
        )
  ...
  Future<void> createRolesAtomicallyIfCollectionEmpty(
    List<AgentRole> roles,
  ) async {
    if (roles.isEmpty) {
      throw const AgentRoleValidationException(
        'Production seed requires at least one agent role.',
      );
    }
    ...
    if (existing.docs.isNotEmpty) {
      throw const AgentRoleValidationException(
        'Production seed refused because agent_roles is not empty.',
      );
    }
    ...
    for (final AgentRole role in roles) {
      ...
      if (!roleIds.add(role.roleId.trim())) {
        throw AgentRoleValidationException(
          'Duplicate production seed roleId: ${role.roleId}.',
        );
      }

      if (role.enabled && role.mode == AgentMode.auto) {
        throw AgentRoleValidationException(
          'Production OFF baseline forbids enabled AUTO role: '
          '${role.roleId}.',
        );
      }
    }
    ...
    for (final AgentRole role in roles) {
      batch.set(_collection.doc(role.roleId), role.toMap());
    }
  ...
  Future<void> createRoleIfNotExists(AgentRole role) async {
    ...
    final DocumentSnapshot<Map<String, dynamic>> existing = await _collection
        .doc(role.roleId)
        .get();
  ...
  Future<void> syncRolePermissions({
    ...
    required List<String> approvalRequiredActions,
    required List<String> forbiddenActions,
    String? description,
  }) async {
    ...
    if (normalizedRoleId.isEmpty) {
      throw const AgentRoleValidationException(
        'roleId is required for permission synchronization.',
      );
    }

    List<String> normalizeActions(List<String> actions, String fieldName) {
      final List<String> normalized = actions
          .map((String action) => action.trim())
      ...
      if (normalized.length != actions.length) {
        throw AgentRoleValidationException(
          '$fieldName contains an empty or duplicate action ID.',
        );
      }
    ...
    final List<String> normalizedAllowed = normalizeActions(
      allowedActions,
      'allowedActions',
    );

    final List<String> normalizedApprovalRequired = normalizeActions(
      approvalRequiredActions,
      'approvalRequiredActions',
    );

    final List<String> normalizedForbidden = normalizeActions(
      forbiddenActions,
      'forbiddenActions',
    );

    final Set<String> allowedSet = normalizedAllowed.toSet();
    final Set<String> approvalSet = normalizedApprovalRequired.toSet();
    final Set<String> forbiddenSet = normalizedForbidden.toSet();

    if (allowedSet.intersection(forbiddenSet).isNotEmpty ||
        approvalSet.intersection(forbiddenSet).isNotEmpty) {
      throw const AgentRoleValidationException(
        'An action cannot be both permitted and forbidden.',
      );
    }

    final AgentRole? existing = await getRole(normalizedRoleId);

    if (existing == null) {
      throw AgentRoleValidationException(
        'Agent role "$normalizedRoleId" not found.',
      );
    }
    ...
    final AgentRole updated = existing.copyWith(
      description: normalizedDescription ?? existing.description,
      allowedActions: normalizedAllowed,
      approvalRequiredActions: normalizedApprovalRequired,
    ...
    await _collection.doc(normalizedRoleId).update(<String, dynamic>{
