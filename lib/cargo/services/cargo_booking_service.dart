...
import 'cargo_commission_service.dart';
...
class CargoBookingService {
  ...
  Future<String> createBooking(CargoBookingModel booking) async {
    ...
    if (!settings.acceptingNewBookings) {
      throw StateError('New Cargo bookings are currently disabled by Admin.');
    }
  ...
  /// Cargo Admin: watch every Cargo booking.
  ///
  /// This is intentionally separate from customer/driver streams
  /// so the Cargo Admin can manage the complete Cargo operation.
  Stream<List<CargoBookingModel>> watchAllBookings() {
  ...
  Future<void> updateStatus({
    ...
    if (status == CargoBookingModel.delivered) {
      await CargoCommissionService().processDeliveredBooking(id);
    }
  }
  ...
  Future<void> setPaymentMethod({
    ...
    final String id = bookingId.trim();
    final String method = paymentMethod.trim().toLowerCase();
    ...
    if (!_supportedPaymentMethods.contains(method)) {
      throw ArgumentError('Unsupported Cargo payment method: $paymentMethod');
    }

    await _bookings.doc(id).update({
      'paymentMethod': method,
      'paymentStatus': 'pending',
      'paymentMethodSelectedAt': Timestamp.now(),
    });
  }
  ...
  Future<void> markPaymentPaid({
    ...
    bool markAdvancePaid = false,
  }) async {
    throw StateError(
      'Real Cargo payment cannot be marked paid from the client. '
  ...
  Future<void> markPaymentFailed({
    ...
    await _bookings.doc(id).update({
      ...
      'paymentFailureReason': _nullableText(failureReason),
      'paymentFailedAt': Timestamp.now(),
    });
  }

  Future<bool> isAdvanceVerified(String bookingId) async {
    final String id = bookingId.trim();
    ...
    if (serviceType != CargoBookingModel.buyForMe) {
      return true;
    }

    return data['advancePaid'] == true && data['paymentStatus'] == 'paid';
  }

  Future<void> ensureAdvanceVerified(String bookingId) async {
    final bool verified = await isAdvanceVerified(bookingId);

    if (!verified) {
      throw StateError('Buy For Me advance payment is not verified.');
    }
  }
  ...
  static const Set<String> _supportedPaymentMethods = <String>{
    'cash',
    'wallet',
    'jazzcash',
    'easypaisa',
