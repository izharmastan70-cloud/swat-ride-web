  ...
  Future<void> processDeliveredBooking(String bookingId) async {
    ...
    for (final doc in driverQuery.docs) {
      ...
      if (application.status == CargoDriverApplicationModel.approved) {
        driverApplicationId = doc.id;
        break;
      }
    }

    // Backward-safe fallback for older bookings where
    // driverId may contain the Cargo application document ID.
    if (driverApplicationId == null) {
      ...
      final Map<String, dynamic>? directData = directDriver.data();

      if (directDriver.exists && directData != null) {
        final CargoDriverApplicationModel application =
            CargoDriverApplicationModel.fromMap(<String, dynamic>{
              ...directData,
              'applicationId': directDriver.id,
            });

        if (application.status == CargoDriverApplicationModel.approved) {
          driverApplicationId = directDriver.id;
        }
      }
    }
    ...
    final DocumentReference<Map<String, dynamic>> driverReference =
        _driverApplications.doc(driverApplicationId);
