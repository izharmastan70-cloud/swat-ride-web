...
import 'cargo_driver_application_service.dart';
...
class CargoOffPlatformReportService {
  ...
  Future<String> submitDriverAskedToCancelReport({
    ...
    if (!settings.offPlatformReportingEnabled) {
      throw StateError(
        'Cargo off-platform reporting is currently disabled by Admin.',
      );
    }
  ...
  Future<void> verifyReport({
    ...
    if (adminAction == CargoOffPlatformReportModel.suspension) {
      final CargoOffPlatformReportModel? report = await getReport(reportId);

      if (report == null) {
        throw StateError(
          'Cargo off-platform report not found after verification.',
        );
      }

      await CargoDriverApplicationService().suspendDriverByUserId(
