...
import '../services/cargo_payment_gateway_service.dart';
...
class _CargoPaymentScreenState extends State<CargoPaymentScreen> {
  ...
  final CargoPaymentGatewayService _paymentGatewayService =
  ...
  Future<void> _confirmPayment() async {
    ...
    try {
      ...
      final double paymentAmount = _isBuyForMe
          ? widget.requiredAdvance
          : widget.deliveryFare;

      final CargoPaymentGatewayResult paymentResult =
          await _paymentGatewayService.startPayment(
            bookingId: widget.bookingId,
            method: method,
      ...
      if (method != 'cash') {
        await _bookingService.markPaymentPaid(
          bookingId: widget.bookingId,
          paymentMethod: method,
          transactionId: paymentResult.transactionId,
          markAdvancePaid: _isBuyForMe,
        );
      }
      ...
      await Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => CargoTrackingScreen(bookingId: widget.bookingId),
        ),
      );
    } catch (error) {
      if (!mounted) {
